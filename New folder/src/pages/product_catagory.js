...
class ProductCatagory extends Component {
    ...
    updateStatus(key, value){
        var change = $.ajax({
            url: 'http://localhost:5000/change_status_product_category',
            method: 'post',
            dataType: 'JSON',
            async:false,
    ...
    editCategory(evt, cate_id){

        var totalEditable = document.getElementsByClassName('editableTag');

        for (let index = 0; index < totalEditable.length; index++) {
            totalEditable[index].removeAttribute("contenteditable");
            totalEditable[index].style.border = null;
        }
  
        var element = document.getElementById(cate_id);
        var contentVal = element.innerHTML;
        ...
        // evt.target.parentNode.previousElementSibling.setAttribute('contenteditable', 'true');
        // evt.target.parentNode.previousElementSibling.style.border = "1px solid green";
    }

    render() { 
        var fetch = (this.fetchCategory());
                                ...
                                <CardBody className="my-card-body">
                                    ...
                                    <div>
                                        <Table striped bordered hover responsive>
                                            ...
                                            <tbody id="myTable">
                                                { fetch.map((category,pos) => (
                                                    <tr key={pos+1}>
                                                        <td>{category.id}</td>
                                                        <td className="editableTag" id={"cate_"+pos}>{category.category}</td>
                                                        <td>
                                                            <button className="btn btn-xs button-radius" style={{borderRadius: '50px'}}><span className="fa fa-barcode "></span></button>&nbsp; 

                                                            {(this.state.edit) ? (<button className="btn btn-xs button-radius" style={{borderRadius: '50px'}}><span style={{color: 'green'}} className="fa fa-check"></span></button>) : (<button className="btn btn-xs button-radius" onClick={(event) => this.editCategory(event, "cate_"+pos)} style={{borderRadius: '50px'}}><span className="fa fa-pencil-square-o"></span></button>)} &nbsp; 

                                                            {category.status == 'active' ? (<button className="btn btn-xs button-radius" onClick={()=>this.updateStatus(category.id,'deactivate')}>Deactivate</button>) : (<button key={category.id} className="btn btn-xs button-radius" onClick={()=>this.updateStatus(category.id,'active')}>Activate</button>)}
